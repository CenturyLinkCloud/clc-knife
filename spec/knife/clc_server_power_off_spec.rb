...
describe Chef::Knife::ClcServerPowerOff do
  ...
  context 'considering displayed information' do
    ...
    context 'without waiting' do
      ...
      it { is_expected.to match(/power off request has been sent/i) }
      it { is_expected.to match(/knife clc operation show #{server_power_off_link['id']}/) }
    end

    context 'with waiting' do
      let(:argv) { valid_argv.concat(%w(--wait)) }

      it { is_expected.to match(/server has been powered off/i) }
  ...
  context 'considering command parameters' do
    context 'when they are valid' do
      ...
      it 'passes them correctly' do
        expect(connection).to receive(:power_off_server).
          with(server_id).
          and_return('operation' => server_power_off_link)
